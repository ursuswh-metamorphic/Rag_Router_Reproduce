...
4. Index technique is now chosen automatically per corpus (no flag needed):
   StatPearls/Textbooks are indexed with 8-bit Scalar Quantization
   (`IndexIVFScalarQuantizer`, ~4x smaller than float32); PubMed/Wikipedia are
   indexed with Binary Quantization (`IndexBinaryFlat`, ~32x smaller) plus a
   float32 memmap sidecar used to rescore the top candidates back to exact L2
   distance. See `fedrag/retriever.py` (`SQ8_DATASETS`, `BQ_DATASETS`).
...
3. SQ8 shards use IVF clustering automatically once a shard has enough vectors; very small shards fall back to a flat (non-IVF) `IndexScalarQuantizer` in `fedrag/retriever.py`.
