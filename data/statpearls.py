    ...
    fnames = sorted(
        [
            ...
            for fname in os.listdir(
                os.path.join(CORPUS_DIR, "statpearls/statpearls_NBK430685")
            )
            if fname.endswith("nxml")
        ]
    )
    if not os.path.exists(os.path.join(CORPUS_DIR, "statpearls/chunk")):
        os.makedirs(os.path.join(CORPUS_DIR, "statpearls/chunk"))
    for fname in tqdm.tqdm(fnames):
        fpath = os.path.join(
            os.path.join(CORPUS_DIR, "statpearls/statpearls_NBK430685"), fname
        )
