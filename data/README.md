...
To map the snippets of each corpus to an embedding space and train and create the corpus index, we use the
[MedCPT Article Encoder](https://huggingface.co/ncbi/MedCPT-Article-Encoder) for corpus passages and the
[MedCPT Query Encoder](https://huggingface.co/ncbi/MedCPT-Query-Encoder) for runtime questions.
Both models are based on BERT-base and produce 768-dimensional dense vectors.
...
query with the MedCPT query encoder, and performs a similarity search between the query vector and the
corpus vectors generated by the MedCPT article encoder.
